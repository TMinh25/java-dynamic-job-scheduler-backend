...
import vn.com.fpt.jobservice.u_service.grpc.BatchRenewalResponse;
...
public class UServiceGrpcImpl {
    ...
    public Object createBatchRenewalContract(Long ticketId, Long phaseId, Long subProcessId) {
        try {
            BatchRenewalRequest request = BatchRenewalRequest.newBuilder().setTicketId(ticketId).setPhaseId(phaseId).setSubProcessId(subProcessId).build();
            BatchRenewalResponse result = uServiceStubClient.createBatchRenewalContract(request);
        } catch (Exception e) {
