...
public class JobServiceImpl implements JobService {
	...
	public boolean scheduleOneTimeJob(String jobName, Class<? extends QuartzJobBean> jobClass, Date date) {
		...
		try {
			...
			log.info("Job with key jobKey :" + jobKey + " and group :" + this.groupKey
	...
	public boolean updateCronJob(String jobName, Date date, String cronExpression) {
		...
		try {
			...
			log.error("SchedulerException while updating cron job with key :" + jobKey + " message :" + e.getMessage());
			return false;
		}
	}
	...
	public boolean unscheduleJob(String jobName) {
		...
		try {
			...
			log.error("SchedulerException while unscheduling job with key :" + jobKey + " message :" + e.getMessage());
			return false;
		}
	}
	...
	public boolean pauseJob(String jobName) {
		...
		String jobKey = jobName;
		JobKey jkey = new JobKey(jobKey, this.groupKey);
		log.info("Parameters received for pausing job : jobKey :" + jobKey + ", groupKey :" + this.groupKey);
		...
		try {
			...
			log.error("SchedulerException while pausing job with key :" + jobName + " message :" + e.getMessage());
			return false;
		}
	}
			...
			for (String groupName : scheduler.getJobGroupNames()) {
				for (JobKey jobKey : scheduler.getJobKeys(GroupMatcher.jobGroupEquals(groupName))) {
					...
					if (triggers.get(0) != null) {
						...
						log.info("Job Name:" + jobName + ", Group Name:" + groupName + ", Schedule Time:"
								+ scheduleTime);
					}
				}
